...
description: End-to-end: import spec → product spec → engineering tasks → implementation → QA (project)
model: claude-opus-4-5
---

# Build From Spec - Complete End-to-End Pipeline

Your goal is to take a user's initial idea/specification and run the complete pipeline: import it, generate a full product spec, create engineering tasks, orchestrate implementation, and verify with QA - all in one flow.
...
**CRITICAL**: You MUST update the Purple CLI status bar using the `purple_update_status` MCP tool at each phase transition. This keeps the user informed of progress.
...
See `@workbench/standards/global/how-agents-document.md` for more details on status reporting.
...
**Status Update**: At the start of Phase 0, call the `purple_update_status` MCP tool with:
- phase: "0 - Validation"
- agent: "orchestrator"
- mode: "plan"
...
**Status Update**: At the start of Phase 1, call the `purple_update_status` MCP tool with:
...
8) **Create Folder Structure**: Derive a feature-slug from $1 and generate the required folder structure as instructed in the documentation standards:
...
**Status Update**: At the start of Phase 2, call the `purple_update_status` MCP tool with:
- phase: "2 - Architecture"
- agent: "engineering-architect"
- mode: "plan"

12) **Read Engineering Standards**: Read all relevant standards from the `./workbench/standards/` directory to understand:
...
13) **Invoke Engineering Architect**: Launch the `engineering-architect` agent with:
   ...
   - Request creation of BOTH:
     - `implementation-spec-{feature-slug}.md`
     - `progress.json` (progress tracking file)

   Wait for the agent to complete and verify that the following files were created:
...
**Status Update**: At the start of Phase 3, call the `purple_update_status` MCP tool with:
- phase: "3 - Implementation"
- agent: "senior-engineer"
- mode: "execute"
- currentTicket: 0
- totalTickets: {actual count from implementation spec}

14) **Read Progress File and Implementation Spec**:
   - Read `{feature-folder}/progress.json` for structured phase/ticket data
   - Read the implementation spec for detailed technical context

   From these files, identify:
   - All phases of implementation (from progress.json)
   - All tickets within each phase (from progress.json)
...
15) **Intelligent Task Grouping**: Analyze tasks to determine optimal grouping:
...
16) **Create Smart Orchestration Plan**: Design an intelligent parallelization strategy:
...
17) **Launch Senior Engineer Agents**: For each phase:
...
18) **Monitor and Coordinate**: After each phase completes:
...
**Status Update**: At the start of Phase 4, call the `purple_update_status` MCP tool with:
- phase: "4 - QA"
- agent: "qa-engineer"
- mode: "review"
...
- If **PASS**: Proceed to step 24 (Manual Setup Documentation)
...
**Status Update**: At the start of Phase 5, call the `purple_update_status` MCP tool with:
...
After QA completes (or is skipped), create `{feature-folder}/manual-setup.md` to document any manual actions the user must perform.
...
- [ ] `EXAMPLE_API_KEY` - Get from [Service Dashboard](https://example.com) and add to `.env`

### Database
- [ ] Run migration: `php artisan migrate` (or equivalent)
...
**Status Update**: At the start of Phase 6, call the `purple_update_status` MCP tool with:
- phase: "6 - Complete"
- agent: "orchestrator"
- mode: "review"
...
Present a summary to the user that prominently highlights the manual setup file:

```
## Build Complete!

**Feature:** {feature-slug}
**QA Status:** {PASS/FAIL/SKIPPED}
...
- Completed Tickets: `completed-tickets-documentation/*.md`
- QA Report: `qa-report-{slug}.md` (if QA ran)
- QA Screenshots: `qa-screenshots/*.png` (if QA ran)
...
- `{feature-folder}/agent-written-specifications/agent-written-product-spec-{feature-slug}.md`
- `{feature-folder}/agent-written-specifications/implementation-spec-{feature-slug}.md`
- `{feature-folder}/progress.json` (progress tracking - updated throughout implementation)
- `{feature-folder}/completed-tickets-documentation/*.md` (one per completed ticket)
- `{feature-folder}/qa-screenshots/*.png` (screenshots from QA verification, if QA ran)
